...
class SyncEngine {
  ...
  Future<void> _consider(IndexEntry remote, PeerLink link) async {
    ...
    if (divergent && _sameContent(local.meta.blockHashes, remote.meta.blockHashes)) {
  ...
  Future<void> _download(IndexEntry remote, String targetPath, String requestPath,
      IndexEntry? local, PeerLink link) async {
  ...
  Future<Uint8List?> _reuseLocal(
      String path, int index, String hash, IndexEntry? local) async {
  ...
  Future<void> _serve(WantBlock want, PeerLink link) async {
    final bytes = await store.readRange(want.path, want.index * blockSize, blockSize);
    await link.send(BlockPayload(want.path, want.index, await cipher.seal(bytes)));
  }

  Future<void> _receive(BlockPayload payload) async {
    final download = _downloads[payload.path];
    if (download == null) return;
    download.blocks[payload.index] =
        Uint8List.fromList(await cipher.open(payload.sealed));
    download.pending--;
