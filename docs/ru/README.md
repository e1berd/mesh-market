...
Mesh Market синхронизирует папки напрямую между устройствами пользователя без центрального сервера. В локальной сети устройства находят друг друга через mDNS multicast. В интернете пиры обнаруживаются через **BitTorrent Mainline DHT** по infohash, производному от секрета роя папки; после этого SDP и ICE-кандидаты обмениваются по DHT-транспорту для установки hole-punched WebRTC data channel. Каждый блок файла шифруется XChaCha20-Poly1305 до выхода с устройства.
